...
function iterateOverWire(instructions, center, board, iteration, intersections) {
    let x = center;
    let y = center;
    let stepsCounter = 0;
    ...
    instructions.forEach(instruction => {
        ...
        for(let counter = +instruction.slice(1); counter > 0; counter -= 1) {
            switch(direction) {
                case "R":
                    x += 1;
                    break;
                ...
                case 3:
                    if (board[x][y] === 2) {
                        intersections.forEach(item => {
                            if (item.intersection[0] === x && item.intersection[1] === y) {
                                item.firstWire = stepsCounter;
                                item.sum = item.firstWire + item.secondWire;
                            }
                        });
                    }
                    break;
...
function findClosestIntersection(intersections, center) {
    let min = 999999;
    intersections.forEach(item => {
        let pair = item.intersection;
        let distance = Math.abs(pair[0] - center) + Math.abs(pair[1] - center);
...
function findIntersectionWithFewerSteps(intersections) {
    let min = 9999999;
    intersections.forEach(item => {
        min = (item.sum < min) ? item.sum : min;
    });

    return min;
}

const [firstWire, secondWire] = parseInput();
const size = calculateSize([...firstWire, ...secondWire]);
const board = createBoard(size);
const center = Math.floor(size/1.5);
intersections = new Array();

iterateOverWire(firstWire, center, board, 1, intersections);
iterateOverWire(secondWire, center, board, 2, intersections);

console.log(findClosestIntersection(intersections, center));

iterateOverWire(firstWire, center, board, 3, intersections);
console.log(findIntersectionWithFewerSteps(intersections));