function getParameters(modes, input, relativeBase, current) {
    const parameters = [];
    for(let which = 0; which < 3; which += 1) {
        switch(modes[which]) {
            case 1:
                parameters.push(current + which + 1);
                break;
    
            case 2:
                parameters.push(relativeBase + input[current + which + 1]);
                break;
    
            default:
                parameters.push(input[current + which + 1]);
                break;
        }
    }
...
input = input.concat(new Array(10000).fill(0));
...
while (input[current] !== 99) {
    ...
    let [a, b, c] = getParameters(paramModes, input, relativeBase, current);
